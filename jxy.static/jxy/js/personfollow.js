...
(function (personfollow) {
    ...
    personfollow.model = (function () {
        ...
        personfollow.resetCurrentData = function (data) {
            personfollow.model.getCurrentData().pk_image = data.pk_image;
            personfollow.model.getCurrentData().image_code = data.image_code;
            personfollow.model.getCurrentData().image_name = data.image_name;
            $("#image_code").val(personfollow.model.getCurrentData().image_code);
            $("#image_name").val(personfollow.model.getCurrentData().image_name);
            $("#pk_image").val(personfollow.model.getCurrentData().pk_image);
        };
        ...
        personfollow.view.new = function () {
            ...
            personfollow.dataTable = window.houoy.public.createDataTable({
                ...
                param: {//查询参数
                    ...
                    pk_folder: function () {
                        return personfollow.model.getCurrentData().pk_folder;
                    }
                },
    ...
    personfollow.controller = (function () {
        ...
        personfollow.controller.toAdd = function () {
            personfollow.model.setModal(window.houoy.public.PageManage.UIModal.CARD);
            personfollow.model.setUIState(window.houoy.public.PageManage.UIState.CREATE);
            personfollow.resetCurrentData({//新增时候当前缓存数据是空
                image_code: null,
                image_name: null
            });
            $("#imageshow").hide();
        ...
        personfollow.controller.toEdit = function () {
            personfollow.model.setModal(window.houoy.public.PageManage.UIModal.CARD);
            personfollow.model.setUIState(window.houoy.public.PageManage.UIState.CREATE);
            personfollow.resetCurrentData(personfollow.dataTable.getSelectedRows()[0]);//设置当前选中的行
            var srcstr = "http://47.94.6.120/personfollow/"+personfollow.model.getCurrentData().path+
                "/"+personfollow.dataTable.getSelectedRows()[0].image_name;
            debugger;
        ...
        personfollow.controller.toList = function () {
            personfollow.model.setModal(window.houoy.public.PageManage.UIModal.LIST);
            personfollow.model.setUIState(window.houoy.public.PageManage.UIState.SEARCH);
            personfollow.model.setSelectState(window.houoy.public.PageManage.DataState.NONE_SELECT);
            personfollow.controller.search();
        };

        personfollow.controller.toCard = function () {
            personfollow.model.setModal(window.houoy.public.PageManage.UIModal.CARD);
            personfollow.resetCurrentData(personfollow.dataTable.getSelectedRows()[0]);//设置当前选中的行
            var srcstr = "http://47.94.6.120/personfollow/"+personfollow.model.getCurrentData().path+
                "/"+personfollow.dataTable.getSelectedRows()[0].image_name;
        ...
        personfollow.controller.saveRow = function () {
            if (!($("#image_name").val()) || !($("#image_code").val())) {
                alert("请填写完整信息");
            } else {

                personfollow.model.getCurrentData().image_name = $("#image_name").val();
                personfollow.model.getCurrentData().image_code = $("#image_code").val();

                var formData = new FormData();
                formData.append("file", $("#imageFile")[0].files[0]);
                if(personfollow.model.getCurrentData().pk_image){
                    formData.append("pk_image", personfollow.model.getCurrentData().pk_image);
                }
                formData.append("image_name", personfollow.model.getCurrentData().image_name);
                formData.append("image_code", personfollow.model.getCurrentData().image_code);
                formData.append("pk_folder", personfollow.model.getCurrentData().pk_folder);
                formData.append("path", personfollow.model.getCurrentData().path);

                $.ajax({
                    url: url + '/personfollow/save',
                    type: 'POST',
                    data: formData,
                ...
                //
                //window.houoy.public.post(url + '/personfollow/save', JSON.stringify(personfollow.model.getCurrentData()), function (data) {
                //    if (data.success) {
                //        alert("保存成功");
                //        personfollow.model.setUIState(window.houoy.public.PageManage.UIState.SEARCH);
                //    } else {
        ...
        personfollow.controller.deleteRow = function () {
            if (confirm('你确定要删除选择项目吗？')) {
                var _ids = [];
                switch (personfollow.model.getModal()) {
                    case window.houoy.public.PageManage.UIModal.CARD:
                        _ids[0] = personfollow.model.getCurrentData().id;
                        break;
                    case window.houoy.public.PageManage.UIModal.LIST:
                        $.each(personfollow.dataTable.getSelectedRows(), function (index, value) {
                            _ids[index] = value.id;
                        });
                        break;
                    default:
                ...
                window.houoy.public.post(url + '/personfollow/delete', JSON.stringify(_ids), function (data) {
                    if (data.success) {
                        personfollow.model.setModal(window.houoy.public.PageManage.UIModal.LIST);
                        personfollow.controller.search();
                    } else {
        ...
        //取消所有行操作
        personfollow.controller.cancelRow = function () {
            personfollow.model.setUIState(window.houoy.public.PageManage.UIState.SEARCH);
            personfollow.resetCurrentData({//新增时候当前缓存数据是空
                image_code: null,
                image_name: null
    ...
    personfollow.initTree = function (onSuccess) {
        var folderTree = null;
                ...
                if (data.success) {
                    ...
                    folderTree = $('#tree').treeview({
                        ...
                        onNodeSelected: function (event, data) {
                            personfollow.model.getCurrentData().select_node_id = data.nodeId;
                            personfollow.model.getCurrentData().pk_folder = data.pk_folder;
                            personfollow.model.getCurrentData().path = getPath(data);
                            //刷新列表区
                            personfollow.controller.search();
                        }
                    });

                    folderTree.treeview('selectNode', [personfollow.model.getCurrentData().select_node_id, {silent: false}]);
                } else {
        ...
        $("#treeSaveBtn").click(function () {
            ...
            window.houoy.public.post(url + '/folder/save', JSON.stringify(paramData), function (data) {
                if (data.success) {
                    personfollow.model.getCurrentData().select_node_id = folderTree.treeview('getSelected')[0].nodeId;
                    loadTree();
                } else {
                    alert("增加失败:" + data.msg);
                }
        ...
        $("#treeDeleteSureBtn").click(function () {
            ...
            window.houoy.public.post(url + '/folder/delete', JSON.stringify(paramData), function (data) {
                if (data.success) {
                    personfollow.model.getCurrentData().select_node_id = 0;
                    loadTree();
                } else {
                    alert("删除失败:" + data.msg);
                }
    ...
    personfollow.registerEvent = function () {
        //注册事件监听
        $("#toAddBtn").click(personfollow.controller.toAdd);
        $("#toEditBtn").click(personfollow.controller.toEdit);
        $("#toCardBtn").click(personfollow.controller.toCard);
        $("#toListBtn").click(personfollow.controller.toList);
        $("#deleteBtn").click(personfollow.controller.deleteRow);
        $("#saveBtn").click(personfollow.controller.saveRow);
        $("#cancelBtn").click(personfollow.controller.cancelRow);
